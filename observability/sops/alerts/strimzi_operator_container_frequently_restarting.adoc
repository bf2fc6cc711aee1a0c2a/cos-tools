...
* Check how may time the strimzi-cluster-operator has been restarted, es example:
+
[source]
----
...
----
+
In this case, the operator get restarted because of the liveness probe fails

* In case the operator get restarted because of the liveness probe fails, check the logs of the strimzi operator POD
+
[source]
----
